...
%Estimation of a different intensity thresold parameter for each channel
if nargin < 5
    temporary_directory = '~/seqFISH_Temporary_file/';
    use_GPU = true;
    Estimated_size_spot = 3;
...
for k=1:l
    ...
    Spot_intensity{k}=table(gather(u(sub2ind(size(u),channel_spot(:,1),channel_spot(:,2),channel_spot(:,3)))));
end
